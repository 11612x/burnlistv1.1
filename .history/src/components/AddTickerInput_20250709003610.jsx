    ...
    <div style={{ display: "flex", alignItems: "center" }}>
      ...
      <button
        onClick={async () => {
          ...
          const existing = JSON.parse(localStorage.getItem("burnlist_watchlists") || "[]");
          const slugMatch = window.location.pathname.split("/").pop();

          const updated = existing.map((w) => {
            if (w.slug === slugMatch) {
              // Ensure newItems are full ticker objects with symbol, buyPrice, buyDate, historicalData
              // newItems already comes from createTicker and should have those fields
              return {
                ...w,

        ...
        onClick={async () => {
          ...
          for (const rawSymbol of validSymbols) {
            ...
            const item = await createTicker(
              ...
              editMode ? buyPrice : undefined,
              editMode ? buyDate : undefined
            );
            if (item) {
              newItems.push(item);
            } else {
          ...
          if (typeof setWatchlists === 'function') {
            setWatchlists((prevWatchlists) => {
              if (!prevWatchlists || typeof prevWatchlists !== "object" || Array.isArray(prevWatchlists)) {
                console.error("❌ prevWatchlists is invalid or not an object:", prevWatchlists);
                return prevWatchlists;
              }

              const updatedObject = { ...prevWatchlists };
              for (const id in updatedObject) {
                if (updatedObject[id].slug === slugMatch) {
                  updatedObject[id] = {
                    ...updatedObject[id],
                    items: [...(updatedObject[id].items || []), ...newItems]
                  };
                }
              }

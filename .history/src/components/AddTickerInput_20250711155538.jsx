...
const AddTickerInput = ({ bulkSymbols, setBulkSymbols, handleBulkAdd, buyDate, setBuyDate, buyPrice, setBuyPrice, setWatchlists, editMode, watchlists }) => {
  ...
  const validateInputs = () => {
    if (!bulkSymbols.trim()) {
      if (touched) setValidationError('Please enter at least one ticker symbol.');
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      {(error || (validationError && (touched || validationError !== 'Please enter at least one ticker symbol.'))) && (
        <NotificationBanner

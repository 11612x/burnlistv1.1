...
const WatchlistChart = ({ portfolioReturnData, height = 300, showTooltip = true, mini = false, suppressEmptyMessage = false }) => {
  ...
  // Find the min and max X (timestamp) for the chart
  const minTimestamp = chartData.length > 0 ? chartData[0].timestampValue : undefined;
  const maxTimestamp = chartData.length > 0 ? chartData[chartData.length - 1].timestampValue : undefined;
  ...
  let yMin, yMax;
  if (mini) {
    // 5% margin for sparkline: highest point 5% below top, lowest 5% above bottom
    const range = maxReturn - minReturn;
    if (range === 0) {
      yMin = minReturn - 1;
      yMax = maxReturn + 1;
    } else {
      yMin = minReturn - range * 0.05;
      yMax = maxReturn + range * 0.05;
    }
  } else {
    yMin = Math.floor((minReturn - 5) * 100) / 100;
    yMax = Math.ceil((maxReturn + 5) * 100) / 100;
    if (yMin === yMax) {
      yMin -= 2;
      yMax += 2;
    }

...
const TickerRow = ({
  ...
  let buy = typeof item.buyPrice === "number" && !isNaN(item.buyPrice)
    ? item.buyPrice
    : (item.historicalData.length > 0 ? item.historicalData[0]?.price : NaN);

  const rawLatestPrice = item.historicalData.at(-1)?.price;
  const latestPrice = (!isNaN(Number(rawLatestPrice))) ? Number(rawLatestPrice) : NaN;
      ...
      <td style={{ padding: 8, fontFamily: "'Courier New', Courier, monospace", color: "#ffffff" }}>
        {
          ...
          (typeof changePercent === "number" && isFinite(changePercent)) ? (
            ...
            (console.warn(`⚠️ Skipping return display for ${item.symbol}: changePercent =`, changePercent), "0.00%")
          )
        }
      </td>
      {editMode && (

...
const TickerTable = ({
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", color: "#fff", background: 'black', fontFamily: 'Courier New', border: '1px solid #7FBAA1' }}>
      <thead>
        <tr style={{ borderBottom: '1px solid #7FBAA1' }}>
          {/* Column headers with clickable sorting functionality */}
          <th onClick={() => handleSort("symbol")} style={{ cursor: "pointer", padding: 8, textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }}>
            Symbol {renderSortArrow("symbol")}
          </th>
          <th onClick={() => handleSort("buyPrice")} style={{ cursor: "pointer", padding: 8, textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }}>
            Buy Price {renderSortArrow("buyPrice")}
          </th>
          <th onClick={() => handleSort("currentPrice")} style={{ cursor: "pointer", padding: 8, textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }}>
            Current Price {renderSortArrow("currentPrice")}
          </th>
          <th onClick={() => handleSort("changePercent")} style={{ cursor: "pointer", padding: 8, textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }}>
            Change % {renderSortArrow("changePercent")}
          </th>
          {/* Show Actions column only if edit mode is enabled */}
          {editMode && <th style={{ padding: 8, textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }}>Actions</th>}
          <th style={{ textAlign: "left", color: "#7FBAA1", borderBottom: '1px solid #7FBAA1', fontFamily: 'Courier New', background: 'black' }} />
        </tr>
      </thead>
      <tbody>
        ...
        {sortedItems.map((item, index) => {
          ...
          const changePercent = !isNaN(latestPrice) && !isNaN(resolvedBuyPrice) && resolvedBuyPrice !== 0
            ? ((latestPrice - resolvedBuyPrice) / resolvedBuyPrice) * 100
            : null;

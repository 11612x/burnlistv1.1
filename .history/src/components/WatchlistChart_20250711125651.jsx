...
const WatchlistChart = ({ portfolioReturnData, showBacktestLine, height = 300, lineColor = '#7FBAA1', hideAxes = false, hideBorder = false, showTooltip = true, referenceLine = null }) => {
  ...
  // Downsample to 30 points (linear)
  const downsampledChartData = downsampleLinear(portfolioChartData, 30);

  if (!downsampledChartData || downsampledChartData.length === 0) {
  ...
  const maxAbsReturn = downsampledChartData && downsampledChartData.length > 0
    ? Math.max(10, ...downsampledChartData.map(p => Math.abs(p.returnPercent)))
    : 10;
  ...
  return (
    <div style={{
      ...
      <ResponsiveContainer width="100%" height="100%" style={{ overflow: "visible", padding: 0, margin: 0 }}>
        ...
        <LineChart data={downsampledChartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
          {/* Y Axis and X Axis */}
...
// Linear downsampling utility: returns up to maxPoints, evenly spaced
function downsampleLinear(data, maxPoints = 30) {

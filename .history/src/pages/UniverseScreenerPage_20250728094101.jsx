...
const UniverseScreenerPage = () => {
  ...
  const [optionsFormOpen, setOptionsFormOpen] = useState(false);
  ...
  const handleJournalClick = (item) => {
    ...
    setTradeType(item.tradeType === 'option' ? 'option' : 'stock');
  ...
  const handleExecuteTrade = (item) => {
    ...
    try {
      ...
      const tradeEntry = {
        id: Date.now(),
        ticker: item.symbol,
        tradeType: item.tradeType || 'stock',
  ...
  const handleOptionsTradeSave = (tradeData) => {
    // Update the universe item with options trade data
    setUniverse(prev => ({
      ...prev,
      items: (prev?.items || []).map(item => {
        if (item.id === journalTicker.id) {
    ...
    setOptionsFormOpen(false);
    setJournalTicker(null);
    setNotification("Options trade saved successfully");
    setNotificationType("success");
  };
...
{(universe?.items?.length || 0) > 0 && (
  <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
    ...
    {universe && (universe?.items?.length || 0) === 0 && (
          ...
          >
            <div style={{ padding: 28, flex: 1, overflowY: 'auto' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                <h3 style={{ marginBottom: '10px', fontSize: '16px' }}>TRADE TYPE</h3>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button
                    onClick={() => setTradeType('stock')}
                    style={{
                      background: tradeType === 'stock' ? CRT_GREEN : 'transparent',
                      color: tradeType === 'stock' ? '#000' : CRT_GREEN,
                      border: `2px solid ${CRT_GREEN}`,
                      fontWeight: tradeType === 'stock' ? 'bold' : 'normal',
                      padding: '8px 16px',
                      fontFamily: 'Courier New',
                      cursor: 'pointer'
                    }}
                  ...
                  <button
                    ...
                    style={{
                      background: tradeType === 'option' ? CRT_GREEN : 'transparent',
                      color: tradeType === 'option' ? '#000' : CRT_GREEN,
                      border: `2px solid ${CRT_GREEN}`,
                      fontWeight: tradeType === 'option' ? 'bold' : 'normal',
                      padding: '8px 16px',
                      fontFamily: 'Courier New',
                      cursor: 'pointer'
                    }}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 16, padding: 20, borderTop: `1.5px solid ${CRT_GREEN}`, background: '#111' }}>
              ...
              {tradeType === 'option' ? (
                <button onClick={() => setOptionsFormOpen(true)} style={{ 
                  fontFamily: 'Courier New',
                  fontWeight: 700,
                  fontSize: 17,
                  borderRadius: 4,
      ...
      <OptionsTradeForm
        item={journalTicker}
        onSave={handleOptionsTradeSave}
        onClose={() => setOptionsFormOpen(false)}
        isOpen={optionsFormOpen}
      />
    </div>
  );
};

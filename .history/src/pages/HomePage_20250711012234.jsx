...
const HomePage = ({ watchlists = {}, setWatchlists }) => {
  ...
  return (
    <div style={{ fontFamily: 'Courier New', color: '#7FBAA1', backgroundColor: 'black', minHeight: '100vh', padding: '32px' }}>
      ...
      <div style={{
        display: view === 'graph' ? 'grid' : 'block',
        gridTemplateColumns: view === 'graph' ? 'repeat(5, 1fr)' : undefined,
        gap: view === 'graph' ? '24px' : undefined,
        margin: view === 'graph' ? '24px 0' : undefined,
        justifyItems: view === 'graph' ? 'center' : undefined,
        alignItems: view === 'graph' ? 'start' : undefined,
      }}>
        ...
        const cardContent = (
          <div style={{
            ...
            {editMode ? (
              ...
              <div style={{ fontSize: 15, color: '#7FBAA1', fontWeight: 'bold', marginBottom: 4 }}>
                {item.name || `WATCHLIST #${idx + 1}`}
              </div>
            )}
            {/* Reason (editable) */}
            {editMode ? (
              ...
              <div style={{ fontSize: 12, color: '#7FBAA1', marginBottom: 4 }}>
                {item.reason || 'N/A'}
              </div>
            )}
            {/* Return */}

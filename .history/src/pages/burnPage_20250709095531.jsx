      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: "30px" }}>
        <AddTickerInput
          ...
          handleBulkAdd={(tickerObjects) => {
            ...
            const parsed = JSON.parse(saved);
            const updatedWatchlists = { ...parsed };

            const current = Object.values(updatedWatchlists).find(w => w.slug === slug);
            if (!current) return;

            current.items = [...(current.items || []), ...tickerObjects];

            updatedWatchlists[current.slug] = current;

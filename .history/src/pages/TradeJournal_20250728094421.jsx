...
const TradeJournal = () => {
  ...
  const [tradeTypeFilter, setTradeTypeFilter] = useState("all"); // "all", "stock", "option"
  ...
  const filteredTrades = trades.filter(trade => {
    ...
    return trade.tradeType === tradeTypeFilter;
  });

  // Get options-specific display value
  const getOptionsDisplay = (trade) => {
    if (trade.tradeType !== 'option') return null;
    
    const strike = trade.strikePrice ? `$${trade.strikePrice}` : '';
    const expiry = trade.expiryDate ? formatDate(trade.expiryDate) : '';
    const optionType = trade.optionType || 'Unknown';
    
    return `${optionType} ${strike} ${expiry}`;
  };

  // Get position size display
  const getPositionDisplay = (trade) => {
    if (trade.tradeType === 'option') {
      return trade.contracts ? `${trade.contracts} contracts` : 'N/A';
    }
    return trade.positionSize ? `${trade.positionSize} shares` : 'N/A';
  };
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        <span style={{ color: CRT_GREEN, fontSize: '12px' }}>FILTER:</span>
        <CustomButton
          onClick={() => setTradeTypeFilter("all")}
          style={{
            background: tradeTypeFilter === "all" ? CRT_GREEN : 'transparent',
            color: tradeTypeFilter === "all" ? '#000000' : CRT_GREEN,
            border: `1px solid ${CRT_GREEN}`,
            padding: '6px 12px',
            fontFamily: "'Courier New', monospace",
            fontSize: '11px',
        ...
        <CustomButton
          onClick={() => setTradeTypeFilter("stock")}
          style={{
            background: tradeTypeFilter === "stock" ? CRT_GREEN : 'transparent',
            color: tradeTypeFilter === "stock" ? '#000000' : CRT_GREEN,
            border: `1px solid ${CRT_GREEN}`,
            padding: '6px 12px',
            fontFamily: "'Courier New', monospace",
            fontSize: '11px',
        ...
        <CustomButton
          onClick={() => setTradeTypeFilter("option")}
          style={{
            background: tradeTypeFilter === "option" ? CRT_GREEN : 'transparent',
            color: tradeTypeFilter === "option" ? '#000000' : CRT_GREEN,
            border: `1px solid ${CRT_GREEN}`,
            padding: '6px 12px',
            fontFamily: "'Courier New', monospace",
            fontSize: '11px',
                ...
                {filteredTrades.map((trade, index) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '0px', textAlign: 'center', borderRight: `1px solid ${CRT_GREEN}`, height: '40px', minHeight: '40px', maxHeight: '40px', lineHeight: '24px', boxSizing: 'border-box', verticalAlign: 'middle', overflow: 'hidden' }}>
                      {trade.tradeType === 'option' ? 'OPTION' : 'STOCK'}
                    </td>
                    {tradeTypeFilter === "option" && (
                      <td style={{ padding: '0px', textAlign: 'center', borderRight: `1px solid ${CRT_GREEN}`, height: '40px', minHeight: '40px', maxHeight: '40px', lineHeight: '24px', boxSizing: 'border-box', verticalAlign: 'middle', overflow: 'hidden', fontSize: '11px' }}>
                        {getOptionsDisplay(trade)}
                      </td>
                    )}
                    <td style={{ padding: '0px', textAlign: 'center', borderRight: `1px solid ${CRT_GREEN}`, height: '40px', minHeight: '40px', maxHeight: '40px', lineHeight: '24px', boxSizing: 'border-box', verticalAlign: 'middle', overflow: 'hidden' }}>

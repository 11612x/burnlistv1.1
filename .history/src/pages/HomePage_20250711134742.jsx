      ...
      {Object.values(watchlists).map((item, idx, arr) => {
        ...
        const cardContent = (
          <div style={{
            ...
            {/* Portfolio info */}
            <div style={{ fontSize: 11, color: CRT_GREEN, marginBottom: 2 }}>
              {tickers.length} stocks | Risk: {riskIndicator}
            </div>
            {/* Best/Worst performer */}
            {bestPerformer && worstPerformer && (
              <div style={{ fontSize: 10, color: CRT_GREEN, marginBottom: 2 }}>
                Best: {bestPerformer.symbol} {bestPerformer.return >= 0 ? '+' : ''}{bestPerformer.return.toFixed(1)}%
              </div>
            )}
            {bestPerformer && worstPerformer && (
              <div style={{ fontSize: 10, color: '#e31507', marginBottom: 2 }}>

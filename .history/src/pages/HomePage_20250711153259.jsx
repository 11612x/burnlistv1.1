...
const HomePage = ({ watchlists = {}, setWatchlists }) => {
  ...
  // Auto-refresh all watchlists every 30 minutes
  const intervalRef = useRef(null);
  useEffect(() => {
    async function fetchAllUniqueTickersAndUpdate() {
      setIsLoading(true);
      setError(null);
      try {
        // 1. Gather all tickers from all watchlists
        const allTickers = Object.values(watchlists)
          .flatMap(wl => (wl.items || []).map(item => item.symbol));
        // 2. Deduplicate
        const uniqueTickers = [...new Set(allTickers.map(s => s.toUpperCase()))];
        // 3. Fetch all unique tickers from API
        const tickerDataMap = {};
        ...
        // 4. Update all watchlists with fresh data
        const updated = {};
        for (const [id, wl] of Object.entries(watchlists)) {
          updated[id] = {
            ...
            items: (wl.items || []).map(item => {
              ...
              if (fresh) {
                // Preserve buyPrice, buyDate, and other custom fields
                return {
                  ...item,
                  ...fresh,
    ...
    intervalRef.current = setInterval(() => {
      fetchAllUniqueTickersAndUpdate();
    }, 30 * 60 * 1000);
    return () => {
  ...
  return (
    <div style={{ fontFamily: 'Courier New', color: CRT_GREEN, backgroundColor: 'black', minHeight: '100vh', padding: '32px' }}>
      ...
      {isLoading && (
        <div style={{ background: '#111', color: '#8CB9A2', padding: '10px', textAlign: 'center', fontFamily: 'Courier New', fontSize: 16, borderBottom: '1px solid #8CB9A2' }}>
          Loading watchlists...
        </div>
      )}
      {error && (
        <div style={{ background: '#e31507', color: 'black', padding: '10px', textAlign: 'center', fontFamily: 'Courier New', fontSize: 16, borderBottom: '1px solid #e31507' }}>
          {error}
        </div>
      )}

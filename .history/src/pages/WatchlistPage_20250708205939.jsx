...
/*
  ...
  */
  ...
  console.log("🧭 Loaded WatchlistPage with slug:", slug);
  // Load persisted watchlists from localStorage if present
  ...
  const watchlistIndex = effectiveWatchlists.findIndex((wl) => wl.slug === slug);
  ...
  useEffect(() => {
    (async () => {
      ...
      const savedPricesKey = `watchlist_prices_${watchlistIndex}`;
      let savedPrices = {};
      try {
        savedPrices = JSON.parse(localStorage.getItem(savedPricesKey)) || {};
      } catch {}
      const initialItems = watchlist.items.map((symbol) => {
        const saved = savedPrices[symbol];
        return {
          symbol,
          price: saved?.price ?? 0,
          returnPercent: saved?.returnPercent ?? 0,
        };
      });
      // Updated buyPrices initialization logic (per-symbol, per-watchlist, persisted)
      const initialBuyPrices = {};
      watchlist.items.forEach((symbol, index) => {
        const buyPriceKey = `burnlist-buyPrice-${watchlistIndex}-${symbol}`;
        const stored = localStorage.getItem(buyPriceKey);
        if (stored !== null) {
          initialBuyPrices[symbol] = stored;
        } else {
          const price = initialItems[index].price.toString();
          initialBuyPrices[symbol] = price;
          localStorage.setItem(buyPriceKey, price);
        }
      });
    ...
    return (
      ...
      >
        <div style={{ display: "flex", alignItems: "center", marginBottom: 20 }}>
          <BackHomeButton top="2rem" left="2rem" />
        </div>
        <WatchlistHeader
          name={"Untitled"}
          averageReturn={0}
        ...
        {/* Chart fallback: always show white border, black bg, Courier */}
        <div style={{
          backgroundColor: "#000000",
          color: "#ffffff",
          border: "2px solid #ffffff",
        ...
        {/* Timeframe selector with strict horizontal layout */}
        <div style={{
          display: "flex",
          flexDirection: "row",
          ...
          {["D", "WEEK", "MONTH", "YTD", "YEAR", "MAX"].map(label => (
            <span key={label} style={{ color: "#ffffff", fontFamily: '"Courier New", Courier, monospace', fontWeight: "bold", fontSize: 16 }}>
              {label}
            </span>
          ))}
        </div>
        <TickerTable
          ...
          selectedTimeframe={"D"}
        />
        {/* Network status: fixed footer label */}
        <div style={{
          position: "fixed",
          bottom: 24,
          right: 24,
  ...
  // Calculate portfolio return data for chart and header
  const filteredSnapshots = historicalSnapshots.filter(
    (s) => Array.isArray(s.historicalData) && s.historicalData.length > 0
  );
  const portfolioReturnData = React.useMemo(() => {
    const data = calculatePortfolioReturns(filteredSnapshots, timeframe);
    console.log("📈 WatchlistPage: Portfolio return data points:", data.length);
    return data;
  }, [filteredSnapshots, timeframe]);
  ...
  // Set watchlist name fallback
  const watchlistName = watchlist?.name || "Untitled";
  // Flag for empty watchlist
  const isEmptyWatchlist = items.length === 0;
  ...
  return (
    ...
    >
      <div style={{ marginBottom: 20 }}>
        <Link to="/" style={{ color: "#0de309", textDecoration: "underline", fontFamily: '"Courier New", Courier, monospace' }}>
      ...
      <div
        style={{
          ...
          marginBottom: 20,
          border: "2px solid",
          borderColor: burnreturn >= 0 ? "#0de309" : "#e31507",
          padding: "0.5rem",
          fontFamily: '"Courier New", Courier, monospace',
          background: "none",
          boxShadow: "none"
      ...
      {/* Chart area: always white border, Courier, no gradients */}
      <div style={{ border: "2px solid #ffffff", padding: 10, marginBottom: 30, background: "#000000", boxShadow: "none" }}>
      ...
      {/* Strict horizontal timeframe selector */}
      <div style={{
        display: "flex",
        flexDirection: "row",
        justifyContent: "center",
      ...
      <TickerTable
        ...
        fontFamily='"Courier New", Courier, monospace'
        backgroundColor="#000000"
        positiveColor="#0de309"
        negativeColor="#e31507"
        labelColor="#ffffff"
      />
      ...
      {/* Network status: fixed bottom right, Courier, white */}
      <div style={{
        position: "fixed",
        bottom: 24,
        right: 24,

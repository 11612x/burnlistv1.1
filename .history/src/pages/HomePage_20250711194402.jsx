...
const HomePage = ({ watchlists = {}, setWatchlists }) => {
  ...
  useEffect(() => {
    if (uniqueRealTickers.length === 0) return;
    
    // Create a virtual "homepage" slug for the fetch manager
    const homepageSlug = 'homepage';
    
    // Function to fetch all unique tickers across all watchlists
    const fetchAllTickers = async () => {
      ...
      const result = await fetchManager.startFetch(homepageSlug, allItems, (updatedItems) => {
    ...
    const interval = setInterval(fetchAllTickers, 30 * 60 * 1000);
    
    return () => {
      clearInterval(interval);
      // Clean up any active fetch for homepage
      fetchManager.cancelFetch(homepageSlug);
    };
  ...
  // Add a manual refresh button handler that calls manualBatchFetch

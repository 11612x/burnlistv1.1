...
const HomePage = ({ watchlists = {}, setWatchlists }) => {
  ...
  // Manual fetch in batches of 60, 1 minute apart, with progress notification
  const manualBatchFetch = async () => {
    ...
    for (let i = 0; i < batches.length; i++) {
      setNotification(`Manual refresh: ${i + 1}/${batches.length}...`);
      setNotificationType('info');
      await new Promise(resolve => setTimeout(resolve, i === 0 ? 0 : 60 * 1000));
      setIsLoading(true);
      setError(null);
      try {
        const tickerDataMap = {};
        for (const symbol of batches[i]) {
          try {
            ...
            if (err?.response?.status === 429) {
              setNotification('API rate limit reached. Waiting 2 seconds...');

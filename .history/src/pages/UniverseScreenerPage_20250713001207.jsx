...
const calculateATR = (high, low, close, period = 14) => {
  // For now, return a placeholder ATR value
  // In a real implementation, you'd calculate this from historical data
  return (high + low + close) / 3 * 0.02; // Simple approximation
};
...
const UniverseScreenerPage = () => {
  ...
  // Load universe from localStorage on mount
  useEffect(() => {
    try {
      const saved = localStorage.getItem("burnlist_universe");
      if (saved) {
        const parsed = JSON.parse(saved);
        setUniverse(Array.isArray(parsed) ? parsed : []);
      }
    } catch (error) {
  ...
  useEffect(() => {
    localStorage.setItem("burnlist_universe", JSON.stringify(universe));
  }, [universe]);
  ...
  const handleBulkAdd = async () => {
    ...
    try {
      ...
      const existingSymbols = new Set(universe.map(item => item.symbol));
      ...
      setUniverse(prev => [...prev, ...newItems]);
      setBulkSymbols("");
  ...
  const handleItemChange = (id, field, value) => {
    setUniverse(prev => prev.map(item => {
      if (item.id === id) {
        const updated = { ...item };
  ...
  const handleDeleteItem = (id) => {
    setUniverse(prev => prev.filter(item => item.id !== id));
  ...
  const handleSelectAll = () => {
    if (selectedItems.size === universe.length) {
      setSelectedItems(new Set());
    } else {
      setSelectedItems(new Set(universe.map(item => item.id)));
    }
  };
          ...
          <table style={{ 
            ...
            <thead>
              <tr style={{ borderBottom: `1px solid ${CRT_GREEN}` }}>
                ...
                <th style={{ padding: "8px", textAlign: "left", borderBottom: `1px solid ${CRT_GREEN}`, background: 'black' }}>Symbol</th>
                <th style={{ padding: "8px", textAlign: "right", borderBottom: `1px solid ${CRT_GREEN}`, background: 'black' }}>Last Price</th>
                <th style={{ padding: "8px", textAlign: "right", borderBottom: `1px solid ${CRT_GREEN}`, background: 'black' }}>ATR(14)</th>
                <th style={{ padding: "8px", textAlign: "center", borderBottom: `1px solid ${CRT_GREEN}`, background: 'black' }}>Flags</th>
                <th style={{ padding: "8px", textAlign: "right", borderBottom: `1px solid ${CRT_GREEN}`, background: 'black' }}>Entry Price</th>
            ...
            <tbody>
              {universe.map((item) => (
                <tr key={item.id} style={{ borderBottom: `1px solid ${CRT_GREEN}` }}>
                  ...
                  <td style={{ padding: "8px", textAlign: "right" }}>${item.stopLoss}</td>
                  <td style={{ padding: "8px", textAlign: "right" }}>${item.takeProfit}</td>
                  <td style={{ padding: "8px", textAlign: "right" }}>{item.positionSize}</td>

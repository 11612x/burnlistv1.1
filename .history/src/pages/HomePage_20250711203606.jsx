...
const HomePage = ({ watchlists = {}, setWatchlists }) => {
  ...
  // Manual fetch using fetch manager
  const manualBatchFetch = async () => {
    console.log('🔍 manualBatchFetch called');
    console.log('📊 uniqueRealTickers.length:', uniqueRealTickers.length);
    console.log('📊 uniqueRealTickers:', uniqueRealTickers);
    
    if (uniqueRealTickers.length === 0) {
      console.log('⚠️ No real tickers to refresh');
      setNotification('No real tickers to refresh');
    ...
    // Create a combined items array from all watchlists
    const allItems = Object.values(watchlists)
      .flatMap(wl => wl.items || [])
      .filter(item => uniqueRealTickers.includes(item.symbol?.toUpperCase()));
    
    if (allItems.length === 0) {
      setNotification('No valid items to refresh');
      setNotificationType('info');
      return;
    }
    ...
    console.log('🚀 Calling fetchManager.startFetch with:', {
      slug: homepageSlug,
      itemsCount: allItems.length,
      items: allItems.map(i => i.symbol)
    });
    
    const result = await fetchManager.startFetch(homepageSlug, allItems, (updatedItems, progress) => {
      console.log('📊 Fetch progress:', progress);
      // Show progress notification
      if (progress) {
        setNotification(`Manual refresh: ${progress}...`);
        setNotificationType('info');
      }
      
      // Update all watchlists with the fresh data
      const updated = {};
      for (const [id, wl] of Object.entries(watchlists)) {
        updated[id] = {
          ...wl,
          items: (wl.items || []).map(item => {
            const fresh = updatedItems.find(i => i.symbol?.toUpperCase() === item.symbol?.toUpperCase());
            if (fresh) {
              return {
                ...item,
                ...fresh,
    ...
    if (result.success) {
      ...
      setNotification('Manual refresh complete!');
      setNotificationType('success');
      setTimeout(() => setNotification(''), 3000);
    } else if (result.message) {
      console.log('⚠️ Fetch failed with message:', result.message);
      setNotification(result.message);
      setNotificationType('info');
    } else {
      console.log('❌ Fetch failed without message');
  ...
  // Check if homepage fetch is active
  const homepageFetchStatus = fetchManager.getFetchStatus('homepage');
  const isHomepageFetching = homepageFetchStatus && homepageFetchStatus.status === 'active';

  // Manual refresh button handler - starts fetch or cancels if active
  const handleManualRefresh = () => {
    console.log('🔄 Manual refresh clicked');
    console.log('📊 uniqueRealTickers:', uniqueRealTickers);
    console.log('📊 isHomepageFetching:', isHomepageFetching);
    
    if (isHomepageFetching) {
      // Cancel the active fetch
      console.log('❌ Cancelling active fetch');
      fetchManager.cancelFetch('homepage');
      ...
      // Start a new fetch
      console.log('🚀 Starting manual batch fetch');
      manualBatchFetch();
    }
  };
    ...
    <div style={{ fontFamily: 'Courier New', color: CRT_GREEN, backgroundColor: 'black', minHeight: '100vh', padding: '32px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <CustomButton
          ...
          style={{
            backgroundColor: isHomepageFetching ? '#e31507' : 'black',
            color: isHomepageFetching ? 'black' : CRT_GREEN,
            border: `1px solid ${isHomepageFetching ? '#e31507' : CRT_GREEN}`,
            cursor: 'pointer',
          }}
        >
          {isHomepageFetching ? 'CANCEL' : 'REFRESH'}
        </CustomButton>

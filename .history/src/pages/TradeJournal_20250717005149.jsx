...
const TradeJournal = () => {
  ...
  const getVerdict = () => {
    ...
    if (failedItems === 0) return { type: "qualified", text: "✅ Qualified" };
    if (failedItems <= 2) return { type: "discretionary", text: "⚠️ Discretionary" };
    return { type: "blocked", text: "❌ Blocked" };
  };
  ...
  const handleSubmit = () => {
    ...
    const newTrade = {
      id: Date.now(),
      ticker: "", // User can add ticker later
              ...
              <tbody>
                {trades.map((trade) => (
                  <tr key={trade.id} style={{ borderBottom: `1px solid ${CRT_GREEN}` }}>
                    ...
                    <td style={{ padding: '10px', border: `1px solid ${CRT_GREEN}` }}>{trade.verdict}</td>
                    <td style={{ padding: '10px', border: `1px solid ${CRT_GREEN}` }}>
                      <select
                        value={trade.outcome}

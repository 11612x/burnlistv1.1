      ...
      <div style={{ display: "flex", justifyContent: "center", marginTop: "30px" }}>
        <AddTickerInput
          ...
          handleBulkAdd={(tickerObjects) => {
            ...
            if (typeof saved !== "string" || saved.trim()[0] !== "{") {
              console.error("❌ Invalid localStorage value for burnlist_watchlists:", saved);
              return;
            }

            const parsed = JSON.parse(saved);
            const currentSlug = slug;
            const current = parsed[currentSlug];

            if (!current || !Array.isArray(current.items)) return;

            current.items.push(...tickerObjects);
            parsed[currentSlug] = current;

            localStorage.setItem("burnlist_watchlists", JSON.stringify(parsed));

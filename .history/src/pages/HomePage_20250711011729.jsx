  ...
  return (
    <div style={{ fontFamily: 'Courier New', color: '#7FBAA1', backgroundColor: 'black', minHeight: '100vh', padding: '32px' }}>
      ...
      {Object.values(watchlists).map((item, idx, arr) => {
        ...
        const cardContent = (
          <div style={{ width: '100%', fontFamily: 'Courier New', background: 'transparent', padding: 0 }}>
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
              {/* Left: Info */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 24, color: '#7FBAA1', marginBottom: 8 }}>
                  WATCHLIST #{idx + 1}
                </div>
                <div style={{ fontSize: 20, color: '#7FBAA1', marginBottom: 8 }}>
                  Reason: {item.reason || 'N/A'}
                </div>
                <div style={{ fontSize: 28, color: '#7FBAA1', marginBottom: 8 }}>
                  Net Assets:  ${item.currentPrice?.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) || '0.00'}
                </div>
                <div style={{ fontSize: 32, color: returnColor, fontWeight: 'bold', marginBottom: 0 }}>
                  Return: {lastReturn >= 0 ? '+' : ''}{lastReturn.toFixed(2)}%
                </div>
              </div>
              {/* Right: Sparkline */}
              <div style={{ width: 220, height: 60, display: 'flex', alignItems: 'center', justifyContent: 'flex-end' }}>
                {tickers.length > 0 ? (
                  <WatchlistChart 
            ...
            {/* Divider */}
            {idx < arr.length - 1 && (
              <div style={{ width: '100%', height: 2, background: '#7FBAA1', opacity: 0.2, margin: '32px 0 0 0' }} />
            )}
          </div>
        );
        return (

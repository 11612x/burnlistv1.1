  ...
  return (
    <div style={{ fontFamily: 'Courier New', color: '#7FBAA1', backgroundColor: 'black', minHeight: '100vh', padding: '32px' }}>
      ...
      {Object.values(watchlists).map((item, idx, arr) => {
        ...
        const cardContent = (
          <div style={{ width: '100%', fontFamily: 'Courier New', background: 'transparent', padding: 0, marginBottom: 18 }}>
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', minHeight: 80, padding: '8px 0' }}>
              {/* Left: Info */}
              <div style={{ flex: 1, minWidth: 0, paddingRight: 12 }}>
                <div style={{ fontSize: 16, color: '#7FBAA1', marginBottom: 2, fontWeight: 'bold' }}>
                  WATCHLIST #{idx + 1}
                </div>
                <div style={{ fontSize: 13, color: '#7FBAA1', marginBottom: 2 }}>
                  Reason: {item.reason || 'N/A'}
                </div>
                <div style={{ fontSize: 18, color: '#7FBAA1', marginBottom: 2, fontWeight: 'bold' }}>
                  Net Assets:  ${item.currentPrice?.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) || '0.00'}
                </div>
                <div style={{ fontSize: 20, color: returnColor, fontWeight: 'bold', marginBottom: 0 }}>
                  Return: {lastReturn >= 0 ? '+' : ''}{lastReturn.toFixed(2)}%
                </div>
              </div>
              {/* Right: Sparkline with frame */}
              <div style={{ width: 120, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', border: '1.5px solid #7FBAA1', borderRadius: 6, background: '#101818', padding: 2 }}>
                {tickers.length > 0 ? (
                  <WatchlistChart 
            ...
            {/* Divider */}
            {idx < arr.length - 1 && (
              <div style={{ width: '100%', height: 1, background: '#7FBAA1', opacity: 0.15, margin: '12px 0 0 0' }} />
            )}
          </div>
        );
        return (

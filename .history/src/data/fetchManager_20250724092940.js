  ...
  async _processBatch(batch, allItems, abortController, slug) {
    ...
    for (const item of batch) {
      ...
      try {
        ...
        // Use the full historical data from Finviz instead of just the current price
        const finvizHistoricalData = newTicker.historicalData;
        
        if (!finvizHistoricalData || finvizHistoricalData.length === 0) {
          console.warn(`⚠️ Skipping ${item.symbol} due to no historical data`);
          continue;
        }

        // Check if we have new data by comparing the most recent entry
        const latestFinvizData = finvizHistoricalData[0];
        const existingLatestData = item.historicalData?.at(0);
        
        if (existingLatestData && latestFinvizData) {
          const isSamePrice = Math.abs(Number(existingLatestData.price) - latestFinvizData.price) < 0.0001;
          const isOlderTimestamp = new Date(latestFinvizData.timestamp) <= new Date(existingLatestData.timestamp);
        ...
        // Replace the entire historical data with Finviz data
        const updatedTicker = {
          ...item,
          historicalData: finvizHistoricalData,
        };

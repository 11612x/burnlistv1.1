...
export function generateVolatileMockWatchlist() {
  ...
  function makeVolatileHistory(start, swings, points, baseTime) {
    const arr = [];
    let price = start;
    let t = baseTime;
    for (let i = 0; i < points; i++) {
      ...
      t += 1000 * 60 * 60 * 24; // 1 day
    }
    return arr;
  }
  const now = Date.now() - 1000 * 60 * 60 * 24 * 30; // 30 days ago
  // Generate histories first so we can use the first price as buyPrice
  const hist1 = makeVolatileHistory(100, [8, -10, 12, -15, 7, -8], 30, now);
  const hist2 = makeVolatileHistory(50, [5, -7, 9, -11, 6, -6], 30, now);
  const hist3 = makeVolatileHistory(200, [15, -20, 18, -22, 10, -12], 30, now);
  const hist4 = makeVolatileHistory(80, [10, -13, 11, -14, 8, -9], 30, now);
  return {
    id: 'mock-volatile',

...
export async function fetchQuote(symbol) {
  try {
    ...
    // Only keep the most recent data point (current market data)
    const currentPrice = historicalData[historicalData.length - 1].price;
    const marketTimestamp = historicalData[historicalData.length - 1].timestamp;
    
    // Create historical data array with only the current data point
    ...
    // Return ticker with only current market data (no historical data)
    return {
      symbol,
      buyPrice: currentPrice, // Current market price as default
      buyDate: marketTimestamp, // Market timestamp as default
      historicalData: currentHistoricalData // Only current data point
    };
  } catch (error) {

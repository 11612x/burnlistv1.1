  ...
  const updatedItems = await Promise.all(
    items.map(async (item) => {
      ...
      try {
        ...
        console.log(`📥 Raw quote data for ${item.symbol}:`, quote);
        console.log(`🧪 Validating quote.currentPrice for ${item.symbol}:`, quote?.currentPrice);
        if (quote == null || typeof quote.currentPrice === 'undefined' || isNaN(Number(quote.currentPrice)) || Number(quote.currentPrice) <= 0) {

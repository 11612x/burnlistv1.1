...
// Check if we can make a request (respect 60/minute limit)
...
function incrementRequestCounter() {
  resetRequestCounter();
  requestCount++;
  console.log(`📊 API Request #${requestCount}/60 (resets in ${Math.ceil((60000 - (Date.now() - lastResetTime)) / 1000)}s)`);
}
...
function isMarketOpen() {
  ...
  if (hours < 9 || (hours === 9 && minutes < 30)) {
    console.log('❌ Market closed: Before 9:30am');
    return false;
  }
  if (hours > 16 || (hours === 16 && minutes > 0)) {
    console.log('❌ Market closed: After 4:00pm');
    return false;
  }
...
export class FetchManager {
  ...
  async startFetch(slug, items, updateCallback, isManual = false) {
    console.log('🚀 startFetch called for:', slug);
    ...
    if (!marketOpen) {
      if (isManual) {
        console.log('⚠️ Market is closed, returning error for manual fetch');
        return { success: false, message: "Market is closed" };
      }
      console.log('⚠️ Market is closed, returning silent failure for auto fetch');
  ...
  getRequestStatus() {
    ...
    return {
      ...
      limit: 60,
      remaining: 60 - requestCount,
      resetIn: Math.ceil((60000 - (Date.now() - lastResetTime)) / 1000)
    };
  }

...
export function getSlicedData(data, timeframe, buyDate, symbol = "?", buyPrice = null) {
  ...
  // For timeframe calculations, ignore buyDate and use pure timeframe window
  const effectiveStart = timeframeStart || new Date(data[0]?.timestamp || now);
  ...
  // Find data points within the timeframe window (ignoring buy date)
  if (data.length >= 1) {
    // For historical data, find the exact price at the custom buy date
    const buyDateObj = new Date(buyDate || data[0].timestamp);
    const now = new Date();
    
    // Find the exact price at the custom buy date from historical data
    let buyDatePrice = null;
    let buyDateIdx = -1;
    
    // First, try to find exact date match
    for (let i = 0; i < data.length; i++) {
      const dataDate = new Date(data[i].timestamp);
      if (dataDate.toDateString() === buyDateObj.toDateString()) {
        buyDatePrice = data[i].price;
        buyDateIdx = i;
        console.log(`✅ Found exact date match for ${symbol} at ${buyDate}: ${buyDatePrice}`);
        break;
      }
    }
    
    // If no exact match, find closest date
    if (buyDatePrice === null) {
      let minDiff = Infinity;
      for (let i = 0; i < data.length; i++) {
        const dataDate = new Date(data[i].timestamp);
        const diff = Math.abs(dataDate - buyDateObj);
        if (diff < minDiff) {
          minDiff = diff;
          buyDatePrice = data[i].price;
          buyDateIdx = i;
      ...
      console.log(`✅ Found closest date for ${symbol} at ${buyDate}: ${buyDatePrice} (diff: ${minDiff}ms)`);
    }
    
    // Find current price (latest data point)
    const currentPrice = data[data.length - 1].price;
    const currentIdx = data.length - 1;
    
    // Use the found historical price at buy date, or fall back to manual buy price
    const startPrice = buyDatePrice !== null ? buyDatePrice : (buyPrice && !isNaN(buyPrice) ? Number(buyPrice) : data[0].price);
    const endPrice = currentPrice;
    
    startPoint = {
      ...data[buyDateIdx !== -1 ? buyDateIdx : 0],
      price: startPrice
    };
    endPoint = {
      ...data[currentIdx],
    ...
    console.log(`✅ Historical price lookup for ${symbol}:`);
    console.log(`✅ Buy date: ${buyDate} → Price: ${startPrice}`);
    console.log(`✅ Current date: ${data[currentIdx].timestamp} → Price: ${endPrice}`);

...
export function getSlicedData(data, timeframe, buyDate, symbol = "?", buyPrice = null) {
  ...
  // Always use buy date and buy price as start point (static)
  const startIdx = binarySearchClosestIdx(data, buyDateObj.getTime());
  startPoint = {
    ...data[startIdx],
    timestamp: buyDateObj.toISOString(),
    price: (buyPrice && !isNaN(buyPrice)) ? Number(buyPrice) : data[startIdx]?.price || 0
  };
  
  // For timeframe calculation, find the appropriate end point
  let endDate = now;
  if (timeframeStart && timeframeStart.getTime() > buyDateObj.getTime()) {
    // Timeframe is shorter than holding period - use timeframe start as comparison point
    endDate = timeframeStart;
  }
  
  // Find the data point for the end date
  const endIdx = binarySearchClosestIdx(data, endDate.getTime());
  endPoint = data[endIdx];
  
  console.log(`✅ Return calculation for ${symbol} (${timeframe}):`);
  console.log(`✅ Buy date: ${buyDateObj.toISOString()} → Buy Price: ${startPoint.price}`);
  console.log(`✅ Timeframe: ${timeframeStart?.toISOString() || 'MAX'}`);
  console.log(`✅ End date: ${endDate.toISOString()} → Price: ${endPoint?.price}`);

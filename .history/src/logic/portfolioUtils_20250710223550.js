export function getAverageReturn(historicalSnapshots, timeframe) {
  ...
  for (const snapshot of historicalSnapshots) {
    ...
    const { startPoint, endPoint } = getSlicedData(historicalData, timeframe, buyDate, symbol);
...
export function getSlicedData(data, timeframe, buyDate, symbol = "?") {
  ...
  // Validate buyDate: check for missing or invalid date, fallback to addedAt, earliest data timestamp, or now
  // Note: snapshot?.addedAt is not available here, so we only fallback to data[0]?.timestamp or now
  if (!buyDate || new Date(buyDate).toString() === 'Invalid Date') {
    console.warn(`⚠️ [getSlicedData] Invalid or missing buyDate for ${symbol}. Falling back to addedAt or first data point.`);
    // buyDate = snapshot?.addedAt || data[0]?.timestamp || new Date().toISOString();
    buyDate = data[0]?.timestamp || new Date().toISOString();
  }

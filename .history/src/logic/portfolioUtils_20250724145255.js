...
export function getSlicedData(data, timeframe, buyDate, symbol = "?", buyPrice = null) {
  ...
  // Find end point based on timeframe (how far back from NOW to look)
  let endDate = now;
  if (timeframeStart) {
    // Use current price for comparison
    endDate = now;
  }
  
  // Find the data point for the end date (always current/latest)

...
export function getSlicedData(data, timeframe, buyDate, symbol = "?") {
  ...
  // Validate buyDate: check for missing or invalid date, fallback to earliest data timestamp or now
  if (!buyDate || new Date(buyDate).toString() === 'Invalid Date') {
    console.warn(`⚠️ [getSlicedData] Invalid or missing buyDate for ${symbol}. Falling back.`);

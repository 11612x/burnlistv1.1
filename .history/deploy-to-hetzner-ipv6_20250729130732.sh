...
# Deploy Burnlist to Hetzner VPS (IPv6)
# Usage: ./deploy-to-hetzner-ipv6.sh [IPv6_ADDRESS]

if [ -z "$1" ]; then
    echo "Usage: ./deploy-to-hetzner-ipv6.sh [IPv6_ADDRESS]"
    echo "Example: ./deploy-to-hetzner-ipv6.sh 2a01:4f8:1c1a:3172::1"
    exit 1
fi

SERVER_IPV6=$1
REMOTE_USER="root"
REMOTE_PASS="9aUUPNAejkrKCp4KhkEP"

echo "🚀 Deploying Burnlist to Hetzner VPS at [$SERVER_IPV6]"
...
# Step 3: Upload to server using IPv6
echo "📤 Uploading to server via IPv6..."
sshpass -p "$REMOTE_PASS" scp -6 -o StrictHostKeyChecking=no burnlist-deploy.tar.gz $REMOTE_USER@[$SERVER_IPV6]:/tmp/

# Step 4: Setup server (run on remote)
echo "🔧 Setting up server..."
sshpass -p "$REMOTE_PASS" ssh -6 -o StrictHostKeyChecking=no $REMOTE_USER@[$SERVER_IPV6] << 'EOF'
    # Update system
...
NGINX_CONFIG
    ...
    echo "🌐 Your app should be available at: http://[$SERVER_IPV6]"
...
echo "📋 Next steps:"
echo "   1. SSH to your server: ssh -6 root@[$SERVER_IPV6]"
